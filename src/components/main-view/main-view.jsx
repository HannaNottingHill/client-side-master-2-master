...
export const MainView = () => {
  ...
  const fetchMovies = async () => {
    try {
      const response = await fetch(
        "https://movieapp-backend8080-395efad185ff.herokuapp.com/movies", // Replace with your Heroku app URL
        {
  ...
  const fetchUserData = async () => {
    ...
    try {
      const response = await fetch(
        `https://movieapp-backend8080-395efad185ff.herokuapp.com/users/${user.username}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const addFavorite = async (movieId) => {
    await fetch(
      `https://movieapp-backend8080-395efad185ff.herokuapp.com/users/${user.username}/movies/${movieId}`,
      {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      }
  ...
  const handleUserUpdate = async (updatedUserData) => {
    try {
      const response = await fetch(
        `https://movieapp-backend8080-395efad185ff.herokuapp.com/users/${user.username}`, // Ensure this is the correct username
        {
          method: "PUT",
          headers: {
